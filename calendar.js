...
const Calendar = {
  ...
  _buildMonth() {
    ...
    for(let day=1;day<=last.getDate();day++) {
      ...
      const cls=['cal-cell',isTod?'today':'',isSel?'selected':'',isFut?'future':'',
        (dow===0||dow===6)?'weekend':'',feiertag?'feiertag':'',
        tagTyp==='urlaub'?'urlaub':'',tagTyp==='krank'?'krank':''].filter(Boolean).join(' ');
      html+=`<div class="${cls}" onclick="Calendar.selectDay('${ds}')">
        <div class="cal-day-num">${day}${dot}</div>
      </div>`;
    }
    html+='</div>';
  ...
  selectDay(dateStr) {
    ...
    document.querySelectorAll('.cal-cell.selected,.week-row.selected').forEach(el=>el.classList.remove('selected'));
